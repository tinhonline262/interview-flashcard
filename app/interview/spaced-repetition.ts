...
export function calculateStatistics(
  ...
  for (const item of allData) {
    if (item.totalReviews === 0) {
      ...
      if (item.lastReviewDate && (!lastReviewDateGlobal || item.lastReviewDate > lastReviewDateGlobal)) {
  ...
  const averageEaseFactor = reviewedItems > 0 ? totalEaseFactor / reviewedItems : 0
 ...
 */
export function exportSpacedRepetitionData(allData: SpacedRepetitionData[]): string {
 ...
 */
export function importSpacedRepetitionData(jsonString: string): SpacedRepetitionData[] {
  try {
 ...
 */
export function resetSpacedRepetition(items: SpacedRepetitionData[]): SpacedRepetitionData[] {
 ...
 */
export function getDifficultyLevel(easeFactor: number): 'easy' | 'normal' | 'hard' | 'very-hard' {
